      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            ...
            {/* Panel Admin (solo visible para Administrador) */}
            {user?.type === 'admin' && (
              <button
            ...
            {/* Panel Asesor (solo visible para Asesor de Ventas) */}
            {user?.type === 'asesor' && (
              <button
                onClick={() => setCurrentView('asesor')}
                className={`px-3 py-1.5 rounded-xl text-xs font-extrabold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer ${
                  currentView === 'asesor'
                    ? 'bg-amber-600 text-white shadow-md'
                    : 'bg-amber-500/10 text-amber-700 hover:bg-amber-600 hover:text-white'
                }`}
                title="Abrir Panel de Asesor de Ventas"
              >
                <span>🧑‍💼</span>
              ...
              <div className="absolute right-0 top-12 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 p-3 z-50 animate-[fadeIn_0.15s_ease]">
                <div className="border-b border-gray-100 pb-2 mb-2">
                  ...
                  <span className="inline-block mt-1 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-800">
                    {user.type === 'admin' ? '🛡️ Administrador' : user.type === 'asesor' ? '🧑‍💼 Asesor de Ventas' : '👤 Cliente'}
                  </span>
                </div>
                ...
                {user.type === 'asesor' && (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs font-bold text-amber-700 hover:bg-amber-50 rounded-xl transition-colors flex items-center gap-2 cursor-pointer"
                  >
                    <span>🧑‍💼</span>
        ...
        <div className="px-4 pb-4 space-y-1 bg-white border-t border-gray-100 shadow-md">
          ...
          {user?.type === 'asesor' && (
            <button
              ...
              className="w-full text-left px-4 py-2.5 rounded-xl text-base font-bold bg-amber-600 text-white flex items-center gap-2 cursor-pointer"
            >
              <span>🧑‍💼</span>
              <span>Panel de Asesor</span>
            </button>
