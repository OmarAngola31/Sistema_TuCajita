/**
 * dbService.js — Capa de acceso a datos (CRUD) entre los componentes y Supabase.
 *
 * Patrón usado en TODAS las funciones de este archivo:
 *   1. Si `supabase` es null (no configurado) -> devolver datos de ejemplo
 *      (defaultOrders, defaultProducts, etc.) para que la UI nunca se rompa.
 *   2. Si hay conexión, intenta leer/escribir en la tabla real en español
 *      (ej: 'producto', 'solicitud') que coincide con supabase_schema.sql.
 *   3. Como respaldo, intenta también una tabla en inglés (ej: 'products',
 *      'orders') por si el proyecto se conectó a un esquema alternativo.
 *   4. Si todo falla (error de red, tabla no existe, etc.), cae en los
 *      datos de ejemplo para no romper la demo.
 *
 * Los datos `default*` (defaultOrders, defaultProducts, defaultStock, etc.)
 * son datos ficticios usados como "fallback" o para pruebas cuando no hay
 * base de datos conectada.
 *
 * `getProducts()` transforma cada fila de la tabla `producto` a un objeto
 * con nombres en inglés (name, price, category...) para que coincida con
 * el formato que usan los componentes de la tienda. Sin embargo, la vista
 * pública (`Products.jsx`) todavía usa datos ESTÁTICOS de
 * `src/data/productsData.js` en lugar de llamar a `getProducts()`. Esto
 * significa que los productos que edite el Administrador desde el panel NO
 * se reflejan automáticamente en la tienda pública. Ver DOCUMENTACION.md,
 * sección "Limitaciones conocidas", para más detalle.
 */
...
// AUTH & PROFILES
// Crea o actualiza (upsert) la fila del usuario en la tabla `usuario` cada
// vez que alguien inicia sesión o se registra. Si la tabla `usuario` no
// existe todavía (proyecto de Supabase sin el schema aplicado), intenta
// como respaldo la tabla `profiles` (esquema alternativo en inglés).
...
// PRODUCTS CRUD
// Obtiene el catálogo de productos desde Supabase (tabla `producto`, con
// join a `categoria` para traer el nombre de categoría). Si Supabase no
// está disponible o la tabla está vacía, devuelve `defaultProducts`
// (datos importados de src/data/productsData.js).
...
// ORDERS CRUD
// Obtiene los pedidos (tabla `solicitud`) con los datos del cliente asociado
// (join a `usuario`). Usado tanto por el Panel de Administrador como por el
// Panel de Asesor de Ventas para listar pedidos con info de contacto.
